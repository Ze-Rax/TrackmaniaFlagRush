...
Mapping for FlagRush is quite different compared to mapping for regular race modes like TimeAttack,
Rounds, etc. There are specific requirements that have to be met for the map to work in the mode.

## 1. Preparations
Download the mapping resources from the [latest release](https://github.com/Ze-Rax/TrackmaniaFlagRush/releases/latest) and place the contents into your gamedata folder, usually located in your documents folder, that is `Documents/Trackmania` or `Documents/Trackmania2020`. This installs the required MapType-script and items you need to create maps for FlagRush. The items are also available ingame in the [Official Trackmania FlagRush Club](https://www.trackmania.com/clubs/42/items/329884).
...
2. At the bottom right, select "Map Options" to open a popup menu. (Crossed wrench + screwdriver icon)
3. Click the first button: "Set Map Type"
4. Select "FlagRushArena" from the "Map type" list
	- If the MapType is not present in the list, select "Pick from file" and point to the `FlagRushArena.Script.txt` located in your gamedata folder (`Scripts/MapTypes/FlagRushArena.Script.txt`).
...
## 3. Landmarks
The gamemode uses landmarks, i.e. spawns, checkpoints and finishes to mark important spots in the map. Some of them have to be present in every map and need to be configured correctly, others are optional. This section explains the landmarks and their requirements.
...
Each team needs exactly one spawn (any "Start" block) and one or more bases (any "Finish" block). There has to be the same amount of bases for both teams.
...
Flag spawns are designated checkpoints that represent potential flag spawns. Any checkpoint in the game can be used as a flag spawn, but we generally recommend using the provided FlagRush flag spawn items (small circular glowing items). There needs to be at least one flag spawn, but you can add as many additional flag spawns to your map as you like. It is recommended to keep it somewhere in the range of 3-5 flag spawns.

The order of flag spawns at which flags are spawned is generally random. Only the initial flag spawn that is used to spawn the first flag of a round is explicitly set in the map as the "Default Flag Spawn". Flag spawns are configured by using the "Edit Block Properties"-tool (See [Configuring landmarks](#4-configuring-landmarks)).

### Global Reset Trigger (Required)
The "Global Reset Trigger" is a small black cube item. It is required for technical reasons and there has to be exactly one such trigger present on a FlagRush map. It should be placed somewhat close to the center of the map. Placing more than one such global reset trigger may cause performance issues and is heavily discouraged!
...
### Configuring Bases
Select the finish block that should be used as a base. In the context menu on the right side click the button for the team that should defend the given base. There has to be at least one base for each team and the same amount of bases per team.
...
The global reset trigger does not have to be configured explicitly. It should just be left in the "Unused" state.
...
## 5. Team Colors
You can customize the team colors if you want them to be different from the default "blue versus red" FlagRush colors. This can be especially useful if you want to go for a different color theme on your map in general. To ensure good visibility of the colors and consistency in our UI, we only allow changing the hue. This means grayscale and shaded team colors are not possible.
...
Note that due to the usage of gradients in our UI, the ingame visuals of the selected colors may look slightly different than expected. The car skins for the teams will be selected according to the hues of the teams automatically.

## 6. Vehicles
You can define what vehicles (i.e. Stadium, Snow, Rally, Desert) should be available to players on the map.
...
By convention we use the following tag to prefix our FlagRush maps to make them visually appear as such:
