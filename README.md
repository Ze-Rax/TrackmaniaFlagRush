...
*FlagRush* is a community made Trackmania gamemode, in which two teams compete in an arena to capture a flag from a flagspawn near the center of the map and score it at the base of the opposing team. The mode is recommended to be played with balanced teams of about 3-5 players each.
...
## Server How-to
Setting up a dedicated server with FlagRush is not much more complicated than setting up a server with a regular official gamemode. This guide will not go into detail on how to set up a regular server, so please refer to [this wiki](https://wiki.trackmania.io/en/dedicated-server) for more details about a general server setup. Instead, we only focus on the steps and differences that are required to host FlagRush on server.
...
The gamemode exposes an extensive list of settings to optionally adjust several aspects of the gameplay experience. These can be configured in the match settings configuration file. Refer to the documentation for a [list of the available settings together with their description](./Documentation/Mode%20settings.md).
